...
func (api *API) getStudents(c echo.Context) error {
	...
	if active != "" {
		...
		if err != nil {
			log.Error().Err(err).Msgf("[api] erro to parse boolean")
...
func (api *API) createStudent(c echo.Context) error {
	...
	if err := studentReq.Validate(); err != nil {
		...
		return c.String(http.StatusBadRequest, "Error to validate student")
	}
...
func (api * API) updateStudent(c echo.Context) error {
  	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		return c.String(http.StatusInternalServerError, "Student not found")
	}

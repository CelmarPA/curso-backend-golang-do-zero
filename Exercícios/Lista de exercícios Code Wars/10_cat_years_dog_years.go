// I have a cat and a dog.

// I got them at the same time as kitten/puppy. That was humanYears years ago.

// Return their respective ages now as [humanYears,catYears,dogYears]

// NOTES:

// humanYears >= 1
// humanYears are whole numbers only
// Cat Years
// 15 cat years for first year
// +9 cat years for second year
// +4 cat years for each year after that
// Dog Years
// 15 dog years for first year
// +9 dog years for second year
// +5 dog years for each year after that

//go:build ignore

package main

import "fmt"

// func CalculateYears(years int) (result [3]int) {
// 	var humanYears, catYears, dogYears int
// 	humanYears += years

// 	if years == 1 {
// 		catYears += 15
// 		dogYears += 15
// 	} else if years == 2 {
// 		catYears += 24
// 		dogYears += 24
// 	} else if years > 2 {
// 		catYears += 24 + (4 * (years - 2))
// 		dogYears += 24 + (5 * (years - 2))
// 	}

// 	result[0] = humanYears
// 	result[1] = catYears
// 	result[2] = dogYears

// 	return result
// }

func CalculateYears(years int) (result [3]int) { // quando você nomeia os valores de retorno na assinatura da função, não precisa escrever nada depois do return
	switch years {
	case 1: result = [3]int{1, 15, 15}
	case 2: result = [3]int{2, 24, 24}
	default: result = [3]int{years, 24 + 4 * (years - 2), 24 + 5 * (years - 2)}
	}
	return 
}

func main() {
	fmt.Println(CalculateYears(1))
	fmt.Println(CalculateYears(2))
	fmt.Println(CalculateYears(10))
}
